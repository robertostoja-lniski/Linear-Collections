...
{
    myfile  << testType << std::endl;
    const clock_t Vbegin_time = clock();
    
    vec.append(0);
    myfile << float( clock () - Vbegin_time ) * MILI_TO_NANO /  CLOCKS_PER_SEC << std::endl;

    const clock_t Lbegin_time = clock();
    
    list.append(0);
    myfile  << float( clock () - Lbegin_time ) * MILI_TO_NANO /  CLOCKS_PER_SEC << std::endl << std::endl;
}
